...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    s = [s for s in paragraphs if select(s) == True]
    if len(s) > k:
        return s[k]
    else :
        return ''
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    mind, word = limit + 1, user_word 
    for words in valid_words:
        diff = diff_function(user_word, words, limit)
        if diff <= limit and diff < mind:
            mind, word = diff, words 
    return word
    # END PROBLEM 5
...
def time_per_word(times_per_player, words):
    ...
    for player_time in times_per_player:
        ...
        for i in range(len(player_time) - 1):
            time.append(player_time[i + 1] - player_time[i])
        times.append(time)
...
def fastest_words(game):
    ...
    for word in word_indices:
        fastest_player, fastest_time = -1, 100000
        for player in player_indices:
            if time(game, player, word) < fastest_time:
                fastest_player, fastest_time = player, time(game, player, word)
