...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for i in range(orig_opponent_deck_length):
            if cards[i].attack == other_card.attack or cards[i].defense == other_card.defense:
                discard_index.append(i)

        opponent.deck.cards = [cards[i] for i in range(orig_opponent_deck_length) if i not in discard_index]
