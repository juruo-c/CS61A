class VendingMachine:
    ...
    def restock(self, number):
        self.good_store += number 
        return 'Current {1} stock: {0}'.format(self.good_store, self.good_name)
    
    def vend(self):
        if self.good_store == 0:
            return 'Inventory empty. Restocking required.'
        if self.funds < self.good_price:
            return 'You must add ${0} more funds.'.format(self.good_price - self.funds)
        else:
            if self.funds == self.good_price:
                self.good_store -= 1
                self.funds = 0
                return 'Here is your {0}.'.format(self.good_name)
            else:
                t = self.funds - self.good_price 
                self.funds = 0
                self.good_store -= 1
                return 'Here is your {0} and ${1} change.'.format(self.good_name, t)
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        wor = self.cents 
        year = Mint.current_year - self.year 
        if year > 50:
            wor += year - 50
        return wor
